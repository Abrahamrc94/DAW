...
aux ESTACION.CODIGO%TYPE;
aux2 ACCESO.ESTACION%TYPE;

CURSOR codigoE IS SELECT CODIGO FROM ESTACION;

CURSOR numAcc IS SELECT count(numero_acceso) FROM ACCESO GROUP BY ESTACION;

BEGIN

FOR v_codigoE IN codigoE LOOP
    aux := v_codigoE.CODIGO;
    FOR v_numAcc IN numAcc LOOP
        aux2 := v_numAcc.NUMERO_ACCESO;
        END LOOP;
