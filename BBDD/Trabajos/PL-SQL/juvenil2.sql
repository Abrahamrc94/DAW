...
BEGIN
    UPDATE ALUMNOS
    SET NOMBRE = SUBSTR(:new.APENOM,INSTR(:new.APENOM,', ',1,1)+2);
    
    UPDATE ALUMNOS
    SET APELLIDO = SUBSTR(:new.APENOM,1,INSTR(:new.APENOM,',',1,1)-1);
    
END;
/


--SELECT SUBSTR(APENOM,1,INSTR(APENOM,',',1,1)-1) AS APELLIDOS FROM ALUMNOS;
